...
const COUPONS = [
  { id:"NEW10000", tag:"신규고객 전용", emoji:"🎁", headline:"첫 방문 감사 쿠폰",
    ...
    code:"FRESH-NEW-10K", expire:"2026.06.30", note:"1인 1회 · 중복 불가",
  ...
  { id:"MEM5000", tag:"기존고객 감사", emoji:"💌", headline:"단골 고객 할인 쿠폰",
    ...
    code:"FRESH-MEM-5K", expire:"2026.06.30", note:"1인 1회 · 중복 불가",
  ...
  { id:"FREE_DEL", tag:"전 고객 대상", emoji:"🚚", headline:"무료배송 쿠폰",
    ...
    code:"FRESH-FREE-DEL", expire:"2026.06.30", note:"1인 1회 · 배송상품 한정",
...
function Barcode({ code }:{ code:string }) {
  ...
  useEffect(()=>{
    ...
    if (typeof JsBarcode!=="undefined") {
      JsBarcode(ref.current, code, { format:"CODE128", width:1.8, height:54, displayValue:false, background:"#fff", lineColor:C.textDark });
    } else {
      const ctx = ref.current.getContext("2d");
      if (!ctx) return;
      ctx.fillStyle="#fff"; ctx.fillRect(0,0,300,54);
      let x=8;
      for (const ch of code) {
        const w=(ch.charCodeAt(0)%5)+2;
        ctx.fillStyle=x%14<7?C.textDark:"#fff";
        ctx.fillRect(x,5,w,44); x+=w+1.5;
      }
    }
  },[code]);
  return <canvas ref={ref} width={300} height={54} className="tw-w-full tw-h-[46px] tw-block" />;
}
        ...
        {CATEGORIES.map(c=>{
          ...
          return (
            <button key={c.id} onClick={()=>onChange(c.id)}
              ...
              style={{ minWidth:56 }}>
              <div className="tw-w-12 tw-h-12 tw-rounded-2xl tw-flex tw-items-center tw-justify-center tw-text-[22px] tw-transition-all"
                style={{
              ...
              <span className="tw-text-[10px] tw-font-bold tw-leading-none"
                style={{ color: on ? C.deep : C.textLight }}>{c.id}</span>
            </button>
          );
        })}
      </div>
...
function WeekCard({ item }:{ item:typeof WEEK_DEALS[0] }) {
  ...
  return (
    <div className="tw-relative tw-bg-white tw-rounded-3xl tw-overflow-hidden tw-flex tw-flex-col"
      ...
      <div className="tw-absolute tw-top-2 tw-left-2 tw-z-10 tw-rounded-lg tw-text-white"
        style={{ background:C.accent, padding:"4px 7px", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <span className="tw-text-[11px] tw-font-black tw-leading-none">{disc}% OFF</span>
      </div>
      {/* 상품 이미지 */}
      <div className="tw-w-full tw-aspect-square tw-overflow-hidden"
        style={{ background:`linear-gradient(145deg,${C.lighter},${C.light})` }}>
        <img
          src={item.img}
          alt={item.name}
      ...
      <div className="tw-p-3 tw-flex tw-flex-col tw-flex-1">
        ...
        <div className="tw-mt-auto tw-pt-2 tw-rounded-xl tw-py-1.5 tw-text-center tw-text-[10px] tw-font-bold"
          style={{ background:C.light, color:C.deep }}>한정 {item.qty}팩</div>
      </div>
    </div>
  );
}
...
function TimeRow({ item }:{ item:typeof TIME_SALES[0] }) {
  const disc = calcDisc(item.orig, item.sale);
  return (
    <div className="tw-flex tw-items-center tw-gap-3 tw-bg-white tw-rounded-2xl tw-px-4 tw-py-3.5"
      style={{ border:`1px solid ${C.border}`, boxShadow:"0 2px 10px rgba(255,111,97,0.07)" }}>
      {/* 상품 이미지 */}
      <div className="tw-w-16 tw-h-16 tw-rounded-2xl tw-overflow-hidden tw-flex-shrink-0"
      ...
      <div className="tw-flex-1 tw-min-w-0">
        ...
        <span className="tw-inline-block tw-text-[10px] tw-font-bold tw-px-2.5 tw-py-0.5 tw-rounded-full"
          style={{ background:C.light, color:C.deep }}>⏱ {item.time}</span>
      </div>
      <div className="tw-text-right tw-flex-shrink-0">
        ...
        <span className="tw-inline-block tw-text-white tw-text-[10px] tw-font-black tw-px-2 tw-py-0.5 tw-rounded-full"
          style={{ background:C.accent }}>{disc}%↓</span>
      </div>
    </div>
  );
}
...
function ArrivalRow({ item, last }:{ item:typeof NEW_ARRIVALS[0]; last:boolean }) {
  return (
    <div className={`tw-flex tw-items-center tw-gap-3 tw-py-3.5 ${last?"":"tw-border-b"}`}
      style={{ borderColor: last ? undefined : C.light }}>
      {/* 상품 이미지 */}
      <div className="tw-w-16 tw-h-16 tw-rounded-2xl tw-overflow-hidden tw-flex-shrink-0"
        style={{ background:C.light }}>
        <img src={item.img} alt={item.name} className="tw-w-full tw-h-full tw-object-cover" />
      ...
      <div className="tw-text-right tw-flex-shrink-0">
        <span className="tw-block tw-text-[9px] tw-font-black tw-rounded-lg tw-px-1.5 tw-py-0.5 tw-mb-1 tw-text-center tw-text-white"
          style={{ background:C.primary }}>NEW</span>
...
/* ══ COUPON TICKET ══ */
function CouponTicket({ c }:{ c:typeof COUPONS[0] }) {
  ...
  return (
    <div className="tw-rounded-3xl tw-overflow-hidden tw-mb-5 tw-bg-white"
      style={{ border:`1px solid ${C.border}`, boxShadow:"0 6px 24px rgba(255,111,97,0.12)" }}>
      <div className="tw-relative tw-px-6 tw-pt-6 tw-pb-6 tw-overflow-hidden" style={{ background:c.heroBg }}>
        <div className="tw-absolute tw-right-4 tw-top-1/2 -tw-translate-y-1/2 tw-text-[80px] tw-opacity-10 tw-select-none tw-pointer-events-none">
          {c.emoji}
        </div>
        <div className="tw-relative">
          <span className="tw-inline-block tw-text-[10px] tw-font-bold tw-px-3 tw-py-1 tw-rounded-full tw-mb-4"
            style={{ background:c.tagBg, color:c.tagColor }}>{c.tag}</span>
          <div className="tw-flex tw-items-center tw-gap-3">
            <span className="tw-text-[34px]">{c.emoji}</span>
            <div>
              <p className="tw-text-white/70 tw-text-[12px] tw-mb-0.5">{c.headline}</p>
              <p className="tw-text-white tw-text-[32px] tw-font-black tw-leading-none">{c.discount}</p>
              <p className="tw-text-[11px] tw-font-bold tw-mt-0.5 tw-text-white/60">할인</p>
            </div>
          </div>
          <p className="tw-text-white/55 tw-text-[11px] tw-mt-3">{c.sub}</p>
        </div>
      </div>
      <div className="tw-relative tw-h-5 tw-flex tw-items-center tw-overflow-hidden" style={{ background:C.lighter }}>
        <div className="tw-absolute -tw-left-4 tw-w-8 tw-h-8 tw-rounded-full" style={{ background:C.page }}/>
        <div className="tw-flex-1 tw-mx-5 tw-border-t-2 tw-border-dashed" style={{ borderColor:C.borderMid }}/>
        <div className="tw-absolute -tw-right-4 tw-w-8 tw-h-8 tw-rounded-full" style={{ background:C.page }}/>
      </div>
      <div className="tw-bg-white tw-px-6 tw-pt-3 tw-pb-6">
        <div className="tw-rounded-2xl tw-py-3 tw-px-4 tw-text-center tw-mb-4" style={{ background:C.lighter }}>
          <p className="tw-text-[9px] tw-tracking-[2px] tw-uppercase tw-mb-1" style={{ color:C.textLight }}>Coupon Code</p>
          <p className="tw-font-mono tw-text-[14px] tw-font-black tw-tracking-widest" style={{ color:C.deep }}>{c.code}</p>
        </div>
        <div className="tw-rounded-xl tw-p-3 tw-mb-4 tw-bg-white" style={{ border:`1px solid ${C.border}` }}>
          <Barcode code={c.code}/>
        </div>
        <div className="tw-flex tw-justify-between tw-text-[10.5px] tw-mb-4" style={{ color:C.textLight }}>
          <span>📅 ~{c.expire}</span><span>{c.note}</span>
        </div>
        <button onClick={copy}
          className="tw-w-full tw-py-3.5 tw-rounded-2xl tw-text-[14px] tw-font-bold tw-text-white tw-transition-all"
          style={{ background: copied ? C.deep : c.heroBg }}>
          {copied ? "✓ 코드 복사 완료" : "쿠폰 코드 복사하기"}
        </button>
      </div>
    </div>
  );
      ...
      <div className="tw-w-full tw-min-h-screen"
        ...
        <header className="tw-sticky tw-top-0 tw-z-50"
          ...
          <div className="tw-flex tw-items-center tw-justify-between tw-px-5 tw-py-3">
            <div className="tw-flex tw-items-center tw-gap-2.5">
              <div className="tw-w-9 tw-h-9 tw-rounded-2xl tw-flex tw-items-center tw-justify-center tw-text-[18px]"
                style={{ background:C.navBg }}>🛒</div>
              <div>
          ...
          <div className="tw-flex tw-px-2">
            {TABS.map((t,i)=>(
              <button key={t} onClick={()=>{ setTab(i); setActiveCat("전체"); }}
                ...
                {t}
                {tab===i && <div className="tw-absolute tw-bottom-0 tw-left-4 tw-right-4 tw-h-[2.5px] tw-rounded-full" style={{ background:C.primary }}/>}
              </button>
            ))}
          </div>
        </header>
        ...
        {tab===0 && (
          <div style={{ paddingBottom:100 }}>
            <div className="tw-relative tw-overflow-hidden" style={{ background:C.heroGrad }}>
              <div className="tw-absolute tw-right-[-50px] tw-top-[-30px] tw-w-[200px] tw-h-[200px] tw-rounded-full"
                style={{ background:"rgba(255,255,255,0.07)" }}/>
              <div className="tw-absolute tw-right-[15px] tw-top-[35px] tw-w-[110px] tw-h-[110px] tw-rounded-full"
                style={{ background:"rgba(255,255,255,0.06)" }}/>
              <div className="tw-relative tw-px-5 tw-pt-7 tw-pb-6">
                <div className="tw-flex tw-items-start tw-justify-between">
                  <div>
                    <div className="tw-inline-flex tw-items-center tw-gap-1 tw-rounded-full tw-px-3 tw-py-1 tw-mb-3"
                  ...
                  <div style={{ display:"flex", gap:8, width:"max-content", animation:"slideLoop 16s linear infinite" }}>
                    {slideItems.map((item,i)=>(
                      <div key={i} className="tw-flex-shrink-0 tw-rounded-2xl tw-flex tw-items-center tw-gap-2"
                        ...
                        <div className="tw-w-8 tw-h-8 tw-rounded-xl tw-overflow-hidden tw-flex-shrink-0"
                          style={{ background:C.light }}>
                          <img src={item.img} alt={item.name} className="tw-w-full tw-h-full tw-object-cover" />
                        </div>
                        <div>
            ...
            {filteredTime.length>0 && (
              <div className="tw-mx-4 tw-mt-8 tw-rounded-2xl tw-overflow-hidden"
                style={{ background:C.bannerGrad, boxShadow:"0 4px 16px rgba(255,111,97,0.22)" }}>
            ...
            {filteredArrival.length>0 && (
              <>
                ...
                <div className="tw-mx-4 tw-bg-white tw-rounded-2xl tw-px-4 tw-py-1"
                  style={{ border:`1px solid ${C.border}`, boxShadow:"0 2px 10px rgba(255,111,97,0.06)" }}>
            ...
            {hasAny && (
              <div className="tw-mx-4 tw-mt-6 tw-rounded-2xl tw-p-4"
                style={{ background:C.lighter, border:`1px solid ${C.border}` }}>
        ...
        {tab===1 && (
          <div className="tw-px-4 tw-pt-5" style={{ paddingBottom:100 }}>
            <div className="tw-bg-white tw-rounded-3xl tw-overflow-hidden"
              style={{ border:`1px solid ${C.border}`, boxShadow:"0 4px 20px rgba(255,111,97,0.09)" }}>
                ...
                <div className="tw-space-y-4">
                  {[
                    ...
                    <div key={r.label} className="tw-flex tw-gap-3 tw-items-start">
                      <div className="tw-w-9 tw-h-9 tw-rounded-xl tw-flex tw-items-center tw-justify-center tw-flex-shrink-0 tw-text-[15px]"
                        style={{ background:C.light, border:`1px solid ${C.borderMid}` }}>{r.icon}</div>
              ...
              <div className="tw-px-5 tw-pb-6">
                <a href="https://map.naver.com/v5/search/%EC%84%9C%EC%9A%B8%20%EA%B0%95%EB%82%A8%EA%B5%AC%20%EB%B4%89%EC%9D%80%EC%82%AC%EB%A1%9C%20304"
        ...
        {tab===2 && (
          <div style={{ paddingBottom:100 }}>
            <div className="tw-px-6 tw-py-7 tw-text-center"
              style={{ background:`linear-gradient(160deg,${C.heroDark} 0%,${C.heroDark2} 100%)` }}>
              <p className="tw-text-[10px] tw-font-bold tw-tracking-[3px] tw-uppercase tw-mb-2 tw-text-white">Coupon Center</p>
              <h2 className="tw-text-[24px] tw-font-black tw-text-white tw-mb-1">할인 쿠폰</h2>
              <p className="tw-text-[12px] tw-text-white/70">쿠폰 코드를 복사 후 매장 직원에게 제시해 주세요</p>
            </div>
            <div className="tw-mx-4 tw-mt-5 tw-rounded-3xl tw-overflow-hidden tw-grid tw-grid-cols-2"
              style={{ border:`1px solid ${C.borderMid}`, boxShadow:"0 2px 12px rgba(255,111,97,0.10)" }}>
              <div className="tw-text-center tw-py-4 tw-px-2"
                style={{ background:"linear-gradient(135deg,#FFD6CF,#FFB5A8)" }}>
                <span className="tw-inline-block tw-text-[9px] tw-font-black tw-px-2 tw-py-0.5 tw-rounded-full tw-mb-2"
                  style={{ background:C.yellow, color:C.yellowText }}>신규고객</span>
                <p className="tw-text-[22px] tw-font-black tw-leading-none tw-mb-0.5" style={{ color:"#D94841" }}>10,000원</p>
                <p className="tw-text-[9px]" style={{ color:"rgba(217,72,65,0.75)" }}>첫 가입 혜택</p>
              </div>
              <div className="tw-text-center tw-py-4 tw-px-2"
                style={{ background:`linear-gradient(135deg,${C.primary},${C.deep})` }}>
                <span className="tw-inline-block tw-text-[9px] tw-font-black tw-px-2 tw-py-0.5 tw-rounded-full tw-mb-2"
                  style={{ background:"rgba(255,255,255,0.20)", color:"#fff" }}>기존고객</span>
                <p className="tw-text-[22px] tw-font-black tw-text-white tw-leading-none tw-mb-0.5">5,000원</p>
                <p className="tw-text-[9px] tw-text-white/70">단골 감사 쿠폰</p>
              </div>
            </div>
            <p className="tw-text-center tw-text-[10.5px] tw-mt-2 tw-mb-5" style={{ color:C.textLight }}>아래에서 해당 쿠폰을 선택해 복사하세요</p>
            <div className="tw-px-4">
              {COUPONS.map(c=><CouponTicket key={c.id} c={c}/>)}
              <div className="tw-bg-white tw-rounded-2xl tw-p-5"
                style={{ border:`1px solid ${C.border}`, boxShadow:"0 2px 8px rgba(255,111,97,0.05)" }}>
                <p className="tw-text-[13px] tw-font-bold tw-mb-3" style={{ color:C.textDark }}>유의사항</p>
                <ul className="tw-space-y-2.5">
                  {["쿠폰은 중복 수령 및 중복 사용이 불가합니다.","첫 가입 시 한 아이디당 1개 발급됩니다.","쿠폰은 1인 1회만 사용 가능합니다.","이벤트 종료 시 예고 없이 조기 마감될 수 있습니다."].map((txt,i)=>(
        ...
        <div className="tw-fixed tw-bottom-0 tw-left-0 tw-right-0 tw-flex tw-justify-center tw-pointer-events-none tw-z-40">
          <div className="tw-w-full tw-max-w-[480px] tw-pointer-events-auto tw-px-4 tw-pb-6 tw-pt-4"
