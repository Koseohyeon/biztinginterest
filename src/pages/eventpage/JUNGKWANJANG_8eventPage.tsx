...
const Reveal: React.FC<{ children: ReactNode; delay?: number; className?: string }> = ({
  ...
  return (
    <div
      ref={ref}
      className={`tw-transition tw-duration-700 tw-ease-out motion-reduce:tw-transition-none ${
        visible ? "tw-translate-y-0 tw-opacity-100" : "tw-translate-y-4 tw-opacity-0"
      } ${className}`}
...
const Disclosure: React.FC<{
  ...
  return (
    <div
      className={`tw-border-t tw-pt-4 tw-mt-5 ${
        isNavy ? "tw-border-white/15" : "tw-border-[#0E1E3F]/10"
      }`}
    >
      <button
        type="button"
      ...
      >
        <span
          className={`tw-text-[15px] sm:tw-text-base tw-font-semibold ${
            isNavy ? "tw-text-[#F1E4C0]" : "tw-text-[#0E1E3F]/80"
          }`}
        >
          {title}
        </span>
      ...
      <div
        className={`tw-grid tw-transition-all tw-duration-300 tw-ease-out ${
          open ? "tw-grid-rows-[1fr] tw-opacity-100 tw-mt-3" : "tw-grid-rows-[0fr] tw-opacity-0"
        }`}
      >
        <div className="tw-overflow-hidden">
          <div
            className={`tw-text-[14px] sm:tw-text-[15px] tw-leading-7 ${
              isNavy ? "tw-text-white/60" : "tw-text-[#0E1E3F]/65"
            }`}
          >
            {children}
          </div>
...
const Eyebrow: React.FC<{ children: ReactNode; tone?: "onNavy" | "onPaper" }> = ({
  ...
  <span
    className={`tw-inline-flex tw-items-center tw-gap-2 tw-text-[13px] sm:tw-text-sm tw-font-bold tw-tracking-[0.14em] ${
      tone === "onNavy" ? "tw-text-[#F1E4C0]" : "tw-text-[#C4202E]"
    }`}
  >
    <span
      className={`tw-h-[6px] tw-w-[6px] tw-rounded-full ${
        tone === "onNavy" ? "tw-bg-[#F1E4C0]" : "tw-bg-[#C4202E]"
      }`}
    />
    {children}
  </span>
...
const SectionTitle: React.FC<{
  ...
  <div className="tw-mb-8 sm:tw-mb-10">
    ...
    <h2
      className={`tw-mt-3 tw-font-display tw-text-[28px] sm:tw-text-[34px] tw-leading-[1.28] tw-font-bold tw-tracking-[-0.01em] ${
        tone === "onNavy" ? "tw-text-white" : "tw-text-[#161B2E]"
      }`}
    >
      {title}
    </h2>
    {desc && (
      <p
        className={`tw-mt-3 tw-text-[16px] sm:tw-text-[17px] tw-leading-8 ${
          tone === "onNavy" ? "tw-text-white/70" : "tw-text-[#161B2E]/65"
        }`}
      >
        {desc}
      </p>
          ...
          <div className="tw-space-y-4">
            {POINT_TIERS.map((tier, i) => (
              <Reveal key={tier.range} delay={i * 90}>
                <div
                  className={`tw-flex tw-items-center tw-gap-5 tw-rounded-[26px] tw-p-5 sm:tw-p-6 ${
                ...
                >
                  ...
                  <div className="tw-min-w-0 tw-flex-1">
                    <p
                      className={`tw-text-[14px] sm:tw-text-[15px] tw-font-medium ${
                        i === POINT_TIERS.length - 1 ? "tw-text-[#0E1E3F]/70" : "tw-text-white/55"
                      }`}
                    >
                      {tier.range}
                    </p>
                    <p
                      className={`tw-mt-0.5 tw-font-display tw-text-[21px] sm:tw-text-[23px] tw-font-bold ${
                        i === POINT_TIERS.length - 1 ? "tw-text-[#8F1620]" : "tw-text-[#F1E4C0]"
                      }`}
                    >
                      {tier.reward}
                    </p>
  ...
  <p className="tw-font-display tw-text-[16px] sm:tw-text-[17px] tw-font-bold tw-leading-snug tw-text-[#0E1E3F]">
    {combo.reward}
      ...
      {/* ============================================================
         ...
         ============================================================ */}
      <div
        className={`tw-fixed tw-inset-x-0 tw-bottom-0 tw-z-50 tw-px-4 tw-pb-4 tw-pt-3 tw-transition-transform tw-duration-300 sm:tw-hidden ${
          stickyVisible ? "tw-translate-y-0" : "tw-translate-y-full"
        }`}
      >
        <div className="tw-mx-auto tw-flex tw-max-w-[520px] tw-gap-2.5 tw-rounded-[20px] tw-bg-white tw-p-2.5 tw-shadow-[0_-8px_30px_-8px_rgba(14,30,63,0.35)] tw-ring-1 tw-ring-[#0E1E3F]/[0.06]">
          <PillButton href={MEMBERS_URL} variant="solid" className="tw-flex-1 !tw-py-3.5">
            멤버스 가입
          </PillButton>
          <PillButton href={SHOP_URL} variant="ghost" className="tw-flex-1 !tw-py-3.5">
            정몰 쇼핑
          </PillButton>
        </div>
      </div>
