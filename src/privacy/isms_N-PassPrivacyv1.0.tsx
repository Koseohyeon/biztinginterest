...
export default function NPassPrivacyPolicy() {
  ...
  const tocItems: TocItem[] = [
    ...
    { num: 2,  label: "개인정보 자동 수집 장치의 설치·운영 및 거부에 관한 사항" },
  ...
  return (
    <div className="tw-bg-[#f5f7fc] tw-min-h-screen">
      ...
      <div className="tw-max-w-[860px] tw-mx-auto tw-px-6 tw-py-12 tw-pb-20">
        ...
        {/* ── 현재버전/이전버전 버튼 추가 ── */}
        <div className="tw-flex tw-justify-end tw-gap-2 tw-flex-wrap tw-mb-4">
          <button
            type="button"
        ...
        <div id="s1">
          <Section num={1} title="개인정보의 처리 목적, 수집 항목, 보유 및 이용기간">
            ...
            <DataTable
              ...
              rows={[
                [
                  "수신자 본인 확인 및 중복 가입 방지",
                  "이름, 휴대전화번호, 이메일 (네이버 소셜 로그인을 통해 네이버로부터 제공받음, 필수)",
                  "동의 철회 또는 목적 달성 후 5일 이내 복구 불가능한 방법으로 파기",
                  "개인정보 보호법 제15조(계약 체결·이행) 제1항 제4호",
                ],
                [
                  "관계 법령에 따른 기록 보관 및 분쟁 조정",
                  "관계 법령에서 정하는 항목",
                  "아래 법령 보존기간표 참조",
                  "개인정보 보호법 제15조(법령상 의무준수) 제1항 제2호",
                ],
              ]}
            />
          ...
          <Section num={2} title="개인정보 자동 수집 장치의 설치·운영 및 거부에 관한 사항">
            ...
            <ItemRow circle="②">
              <span>
                ...
                서비스 이용과정에서 IP Address, 접속 일시, 서비스 이용 기록이 자동으로 생성·수집될 수 있습니다.
              </span>
            </ItemRow>
            <ItemRow circle="③">
              <span>
                ...
                <NoteBox>
                  ...
                  <div className="tw-mt-3 tw-space-y-3">
                    {[
                      ...
                      <div key={gi}>
                        <div className="tw-text-[13px] tw-font-semibold tw-text-[#081f4a] tw-mb-1">{group.title}</div>
                        <div className="tw-space-y-1">
                          {group.rows.map((txt, i) => (
        ...
        <div id="s3">
          <Section num={3} title="개인정보의 제3자 제공">
            ...
            <DataTable
              headers={["제공받는 자", "제공 목적", "제공하는 항목", "보유 및 이용기간", "법적 근거"]}
              rows={[
                [
                  "비즈팅 N-Pass 서비스 이용 시 명시적으로 안내 및 동의를 받은 고객사(캠페인 주관 광고주). 구체적인 고객사 명칭은 동의 화면 내에 명시.",
                  "N-pass 솔루션을 통한 회원 유치 및 리텐션 마케팅",
                  "이름, 휴대전화번호, 이메일",
                  "제공받는 자(고객사)의 이용 목적 달성 시 또는 수신자의 동의 철회(파기 요청)시 지체 없이 파기",
                  "개인정보 보호법 제17조(정보주체의 동의) 제1항 제1호",
                ],
              ]}
            />
            <NoteBox>
          ...
          <Section num={7} title="개인정보의 안전성 확보조치">
            ...
            <ItemRow circle="③">
              <span>
                ...
                <div className="tw-mt-2 tw-space-y-1">
                  <NumRow label="1)">회사는 개인정보가 저장·보관된 전산실, 자료보관실 등에 대한 출입통제 절차를 수립·운영하고, 권한이 없는 자의 접근을 제한하고 있습니다.</NumRow>
                  <NumRow label="2)">회사는 개인정보가 포함된 서류, 보조저장매체 등을 잠금장치가 있는 안전한 장소에 보관하고 있습니다.</NumRow>
                  <NumRow label="3)">회사는 개인정보가 포함된 서류 및 보조저장매체를 파기하는 경우 복구·재생되지 않도록 안전한 방법으로 파기하고 있습니다.</NumRow>
                </div>
              </span>
            </ItemRow>
          </Section>
        ...
        <div id="s12">
          <Section num={12} title="개인정보 처리방침의 변경">
            <div className="tw-flex tw-gap-6 tw-flex-wrap tw-bg-blue-50 tw-border tw-border-blue-200 tw-rounded-xl tw-px-5 tw-py-4 tw-text-[14px] tw-font-medium tw-text-[#081f4a] tw-mb-4">
              <span><span className="tw-font-normal tw-text-slate-400 tw-mr-1">공고일자</span>2026. 08. 26.</span>
              <span><span className="tw-font-normal tw-text-slate-400 tw-mr-1">시행일자</span>2026. 09. 26.</span>
            </div>
            <ItemRow circle="①">
              <span>
