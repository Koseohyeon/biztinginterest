...
const S1_CONSENT_ROWS: ReactNode[][] = [
  [
    "N-Pass 솔루션을 통한 관심고객 정보를 캠페인을 집행한 고객사에게 전달",
    "이름, 전화번호, 이메일, 네이버회원 식별정보",
    "개인정보 수집·이용 동의일로부터 서비스 이용 기간 동안 보유·이용하며, 회원 탈퇴 시 파기",
    "개인정보 보호법 제15조 (정보주체의 동의) 제1항 제1호",
  ],
  [
    "신규 서비스 출시 안내, 이벤트 및 프로모션 정보 제공, 맞춤형 혜택 안내 등 광고성 정보 전달 (이메일, 문자메시지, 카카오 메시지)",
    "[선택] 서비스 담당자 정보(성명, 전화번호, E-mail)",
...
const CHANGE_ROWS: [string, string][] = [
  ["관심고객 정보 전달 항목 추가", "관심고객 정보 전달(이름, 전화번호, 이메일, 네이버회원 식별정보) 추가"],
  ["광고성 정보 수신(선택) 추가", "신규 서비스·이벤트·프로모션 등 광고성 정보 전달"],
  ["제3자 제공 항목 수정", "제공 목적(관심 고객 정보 전달, 이벤트 안내) 및 법적 근거(제17조 제1항 제1호) 수정"],
];

type BiztingPrivacyV11DocumentProps = {
  modal?: React.ComponentProps<typeof ClauseStaticDocument>["modal"];
  mode?: "scheduled" | "current";
};

const BiztingPrivacyV11Document = ({ modal, mode = "current" }: BiztingPrivacyV11DocumentProps) => {
...
export default BiztingPrivacyV11Document;
